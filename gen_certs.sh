...
openssl genrsa -out certs/ca.key 4096 -days 36500

# GENERATE A CA CERT WITH THE PRIVATE KEY
openssl req -new -x509 -nodes -key certs/ca.key -sha256 -out certs/ca.crt -subj "/CN=Admission Controller Webhook" -days 36500

# CREATE THE PRIVATE KEY FOR OUR SERVER
openssl genrsa -out certs/server.key 2048 -days 36500
...
openssl x509 -req -in certs/server.csr -CA certs/ca.crt -CAkey certs/ca.key -CAcreateserial -out certs/server.crt -sha256 -extfile certs/server_config.txt -extensions req_ext -days 36500
